var EventListView = (function() {
	...
	var listContainerId;
	var page;
	function EventListView(eventDao, listContainerId) {
		dao = eventDao;
		listContainerId = listContainerId;
		self = this;


		this.init = function(numberOfPage) {
			page = numberOfPage;
			document.getElementById(listContainerId).innerHTML = '<h4 id="no-events-found"\
			style = "text-align: left; margin-top: 50px;" > No\
			se han encontrado eventos recientes</h4>';
			insertEventList($('#' + listContainerId));
			dao.listEvent(page, function(events, textStatus, request) {
	...
	var createEventRow = function(event) {
		...
		return '<div class="col-sm-4 p-3">\
				<div class="card p-3" onClick="showEvent(' + event.id + ', '+ page +', null)">\
	...
	var createPagination = function(page, parent, totalPages) {
		...
		$('#last_page_link').click(function() {
			self.init(parseInt(totalPages) - 1);
		});
	}
